...
def make_annot_files(args, bed_for_annot):
    ## how this script was written, it was making a bim bed with only chrom and pos, interseting
    ## with the annot file and merging back with the bim and adding 0 and 1.
    ## problem arizes in hg38 1000g bim file eg at chr 10 pos 17838233 where there are two rsid entries in the bim rs546174412 and rs587766952
    ## Either, these should be removed, or the bim bed should also contain rsid
    ## Modifying to include the rsid in the bed and merge back with it.
    print('making annot file')
    df_bim = pd.read_csv(args.bimfile,
            delim_whitespace=True, usecols = [0,1,2,3], names = ['CHR','SNP','CM','BP']).drop_duplicates()
    iter_bim = [['chr'+str(x1), x2 - 1, x2, x3] for (x1, x2, x3) in np.array(df_bim[['CHR', 'BP', 'SNP']])]
    bimbed = BedTool(iter_bim)
    annot_intersect = bimbed.intersect(bed_for_annot, wao=True).to_dataframe(names=['chrom','start','BP','SNP','chrom1','start1','end1','ANNOT'])[['chrom','start','BP','SNP','ANNOT']]
    assert df_bim['BP'].astype(int).equals(annot_intersect['BP'].astype(int))
    df_annot = annot_intersect[['ANNOT']]
