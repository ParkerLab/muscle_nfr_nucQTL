configfile: "coloc_gwas_grip_strength.yaml"
GWAS_annots=["grip_strength"]
ids=config["IDS"]
...
LD_files = get_LD_file("../../results/results_09232024/gwas_ukbb/continuous-46-both_sexes-irnt/continuous-46-both_sexes-irnt_leadvar.tsv")
...
rule all:
    input:
        ...
        #expand("../../results/results_10212024/{GWAS_annot}_susie_models/{LD_file}.susie.Rda", GWAS_annot=GWAS_annots, LD_file=LD_files),
        #expand("../../results/results_07142023/{GWAS_annot}_susie_models/{LD_file}_alignment.png", GWAS_annot=GWAS_annots, LD_file=LD_files),
        expand("../../results/results_10282024/GWAS_{GWAS_annot}_coloc/{celltype}_{region}/{LD_file}_res_summary_{filter_window}win.Rda", celltype=celltypes, region=regions, GWAS_annot=GWAS_annots, LD_file=LD_files, filter_window=filter_window),
...
rule runsusie:
    input:
        "../../results/results_09232024/gwas_ukbb/continuous-46-both_sexes-irnt/continuous-46-both_sexes-irnt_hg38.h.tsv"
    params:
    ...
    output:
        "../../results/results_10212024/{GWAS_annot}_susie_models/{LD_file}_susie_dat.Rda"
    conda:
        "R423"
    envmodules:
        "Bioinformatics",
...
rule coloc:
    input:
        "../../results/results_10212024/{GWAS_annot}_susie_models/{LD_file}.susie.Rda",
    params:
        LD_file = "{LD_file}",
        celltype="{celltype}",
        GWAS_annot="{GWAS_annot}",
        ...
        region="{region}",
    output:
        "../../results/results_10282024/GWAS_{GWAS_annot}_coloc/{celltype}_{region}/{LD_file}_res_summary_{filter_window}win.Rda"
    conda:
        "R423"
    envmodules:
        "Bioinformatics",
...
rule tidy_coloc:
    input:
        "../../results/results_10282024/GWAS_{GWAS_annot}_coloc/{celltype}_{region}/{LD_file}_res_summary_{filter_window}win.Rda"
    output:
        "../../results/results_10282024/gwas_coloc_plots_{celltype}_{filter_window}win/{GWAS_annot}_{region}/locuscomapreplot/logs/{LD_file}_win_PPH4{PPH4_threshold}.tsv"
    params:
        LD_file = "{LD_file}",
        celltype="{celltype}",
        outdir_coloc=config["outdir_coloc"],
...
rule coloc_summary:
    ...
    params:
        GWAS_annot="{GWAS_annot}",
        rda_dir = '../../results/results_10282024/GWAS_{GWAS_annot}_coloc/',
        filter_window=config["filter_window"],
