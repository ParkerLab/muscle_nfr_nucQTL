...
snakemake all  --jobname "${pipeline}.{jobid}" --jobs 1000 \
                ...
                --cluster "sbatch --account {cluster.account} --output {cluster.output} --time {cluster.time} --ntasks {cluster.ntasks} --cpus-per-task {cluster.cpus} --mem-per-cpu {cluster.mem} --parsable" \
