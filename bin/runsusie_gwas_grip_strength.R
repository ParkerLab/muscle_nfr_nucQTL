...
gwas_stats_chr = gwas_stats %>% filter(`hg38_chrom` == Chr)
...
for (j in 1: nrow(coloc_gwas_df)){
    ...
    if (nrow(vcf_snp_df) == 0) {
        ...
        beta_modified = c(beta_modified, coloc_gwas_df[j,]$beta_EUR)
        ...
        if (any(fullid == vcf_snp_df$pos_id_ref_alt)) {
            ...
            # keep the beta
            beta_modified = c(beta_modified, coloc_gwas_df[j,]$beta_EUR * (-1.0))
            # keep the snp
            keep_list = c(keep_list, 1)
        } else {
            beta_modified = c(beta_modified, coloc_gwas_df[j,]$beta_EUR)
...
coloc_gwas_df = coloc_gwas_df[match(names(LD_dat), coloc_gwas_df$hm_rsid),]
coloc_d1 <- list()
coloc_d1$beta = coloc_gwas_df$beta_modified
coloc_d1$varbeta = (coloc_gwas_df$se_EUR)^2
