...
option_list <- list(
    ...
    make_option(c("--indir_gwas"), type = "character", help = "[Required] Location of the GWAS susie models"),
...
# if no sigqtls
if ((nrow(NFR_models_summary) == 0) | chr == 'chrX'){
    # track lists
    df_res_nfr = as.data.frame(matrix(nrow=0,ncol=12))
    options(scipen=999)
    save(df_res_nfr, file = paste0(outdir_coloc, "gwas_", gwas_peak, "_res_summary_", filter_window, "win.Rda"))
...
gwas_mid = (strtoi(strsplit(gwas_peak, split = "-")[[1]][2])+strtoi(strsplit(gwas_peak, split = "-")[[1]][3]))/2

print(paste0("---------GWAS and ", region, "QTLs---------"))
# track lists - nfr
df_res_nfr_list = list()
...
if (nrow(nearby_peaks) > 0){
    for (i in 1:nrow(nearby_peaks)){
        ...
        load(gwas_dat)
        S_gwas = S1
        # if gwas is null or contains NA
        if (is.null(summary(S_gwas))){
            print('S_gwas is invalid or contains NA, skipping iteration.')
            next
        }
        load(S_nfr_path)
        S_nfr = S1
        # if nfr is null or contains NA
        if (is.null(summary(S_nfr))){
            print('S_nfr is invalid or contains NA, skipping iteration.')
            next
        }
        # run coloc
        ...
        if (nrow(summary_df) > 0) {
            ...
            # bind
            df_res_nfr_list[[length(df_res_nfr_list) + 1]] = summary_df
        }
    }
}
# save summary
df_res_nfr = do.call(rbind, df_res_nfr_list)
options(scipen=999)
save(df_res_nfr, file = paste0(outdir_coloc, "gwas_", gwas_peak, "_res_summary_", filter_window, "win.Rda"))
